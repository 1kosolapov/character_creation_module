"""Это короткая игра.
Состоит из виртуального окружения venv.
autor: Виктор Косолапов
mail: vikt.kosolapoff@yandex.ru.
"""
...
def attack(char_name: str, char_class: str) -> str:
    """Функция, которая вычисляет атаку."""
...
def defence(char_name: str, char_class: str) -> str:
    """Функция, которая вычисляет защиту."""
    if char_class == 'warrior':
...
def special(char_name: str, char_class: str) -> str:
    """Специальная фунуция, учитывающая таланты персонажа."""
    if char_class == 'warrior':
        return (
...
def start_training(char_name: str, char_class: str) -> str:
    """Функция тренировки."""
    if char_class == 'warrior':
        print(f'{char_name}, ты Воитель — отличDный боец ближнего боя.')
    if char_class == 'mage':
    ...
    print('Введи одну из команд: attack — чтобы атаковать противника, ',
          ...
          'pecial — чтобы использовать свою суперсилу.')
